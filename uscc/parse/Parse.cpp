...
// Helper functions to report syntax warnings
void Parser::reportSemantWarn(const std::string& msg, int colOverride, int lineOverride) noexcept
{
	if (mCheckSemant)
	{
		int col;
		...
		mWarns.push_back(make_shared<Warn>(msg, line, col));
	}
}

void Parser::displayWarnMsg(const std::string& line, std::shared_ptr<Warn> warn) noexcept
{
	(*mWarnStream) << mFileName << ":" << warn->mLineNum << ":" << warn->mColNum;
	(*mWarnStream) << ": warn: ";
	(*mWarnStream) << warn->mMsg << std::endl;

	(*mWarnStream) << line << std::endl;
	// Now add the caret
	for (int i = 0; i < warn->mColNum - 1; i++)
	{
		if (line[i] == '\t')
		{
			(*mWarnStream) << '\t';
	...
	(*mWarnStream) << '^' << std::endl;
}

void Parser::displayWarns() noexcept
{
	// Output warns
	// Move the filestream back to the start
	int lineNum = 0;
...
Identifier* Parser::getVariable(const char* name) noexcept
{
	...
	if (!ident)
	{
		reportSemantError(std::string("Use of undeclared identifier '") + std::string(name) + "'");
...
std::shared_ptr<ASTExpr> Parser::charToInt(std::shared_ptr<ASTExpr> expr) noexcept
{
	...
	{
		if (auto e = std::dynamic_pointer_cast<ASTConstantExpr>(expr))
		{
			e->changeToInt();
			retVal = e;
		}
...
std::shared_ptr<ASTExpr> Parser::intToChar(std::shared_ptr<ASTExpr> expr) noexcept
{
	...
	{
		if (auto e = std::dynamic_pointer_cast<ASTToIntExpr>(expr))
			return e->getChild();
		if (auto e = std::dynamic_pointer_cast<ASTConstantExpr>(expr))
		{
			e->changeToChar();
