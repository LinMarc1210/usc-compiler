...
public:
	...
	bool checkShadow(const char* name);

    bool checkReturnValOfFunc(const char* name);

    bool checkArrayIdx(const char* name, std::shared_ptr<ASTExpr> expr);
	...
	public:
		...
		Identifier* searchShadow(const char* name) noexcept;
