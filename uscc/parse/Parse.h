...
public:
	...
	bool IsWarn() const noexcept
	{
		return mWarns.size() == 0;
	}
...
protected:
	...
	// Helper function to report a semantic error
	// (These only display if mCheckSemant == true)
	void reportSemantWarn(const std::string& msg, int colOverride = -1,
						   int lineOverride = -1) noexcept;
	
	// Struct used to store an error
	struct Warn
	{
		Warn(const std::string& msg, int lineNum, int colNum)
		: mMsg(msg)
		, mLineNum(lineNum)
		, mColNum(colNum)
		{ }
	...
	// Write an error message to the error stream
	void displayWarnMsg(const std::string& line, std::shared_ptr<Warn> warn) noexcept;
	
	// Writes out all the error messages
	void displayWarns() noexcept;
...
protected:
	...
	std::shared_ptr<ASTStmt> parseAssignStmt(bool isForStep = false);
	...
	std::shared_ptr<ASTDoWhileStmt> parseDoWhileStmt();
	std::shared_ptr<ASTSwitchStmt> parseSwitchStmt();
	std::shared_ptr<ASTCaseDefaultStmt> parseCaseStmt(std::shared_ptr<ASTExpr> switch_var);
    std::shared_ptr<ASTBreakStmt> parseBreakStmt();
    std::shared_ptr<ASTContinueStmt> parseContinueStmt();
...
private:
	...
	// Ostream warnings should be output to
	std::ostream* mWarnStream;
	...
	// List used to store all of the warns
	std::list<std::shared_ptr<Warn>> mWarns;
