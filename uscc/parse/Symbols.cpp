...
// Searches this scope first, and if not found searches
// through parent scopes. Returns nullptr if not found.
Identifier* SymbolTable::ScopeTable::searchShadow(const char* name) noexcept
{
	// PA2: Implement Extended: (Shadowing of Identifiers)
	auto ret = searchInScope(name);
    if (ret && !ret->isFunction()) {
        ...
        if (mParent) {
            return mParent->searchShadow(name);
        }
        else {
            return nullptr;
        }
...
bool SymbolTable::checkShadow(const char* name)
{
	// PA2: Implement Extended: (Shadowing of Identifiers)
    auto mParent = mCurrScope->getParent();
	if (mParent)
		return mParent->searchShadow(name) != nullptr;
    else
		return false;
}

bool SymbolTable::checkReturnValOfFunc(const char* name)
{
	// PA2: Implement Extended: (Ignoring the Return Value of a Non-Void Function)
	auto ret = mCurrScope->search(name);
    if (ret) {
        if (ret->isFunction()) {
            std::shared_ptr<ASTFunction> retFunction = ret->getFunction();
            if (retFunction) {
                return retFunction->getReturnType() != Type::Void;
            }
            else {
                return false;
            }
...
bool SymbolTable::checkArrayIdx(const char* name, std::shared_ptr<ASTExpr> expr)
{
	// PA2: Implement Extended: (Constant folding in array size / subscripts)
    std::shared_ptr<ASTConstantExpr> constExpr;
    if (constExpr = std::dynamic_pointer_cast<ASTConstantExpr>(expr)) {
        int inputidx = constExpr->getValue();
        auto searched = mCurrScope->search(name);
        size_t arraysize = searched->getArrayCount();

        if (inputidx < 0 || inputidx > arraysize-1) {
            return true;
        }
        else {
            return false;
